...
class UserDao extends BaseDao
{
    ...
    {
        if ($is_list)
            return $this->searchWhere($where)->group('uid')->fetchSql(true)->column('count(*) as user_count', 'uid');
        else
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getAdminUserList(array $where, string $field = '*', int $page = 0, int $limit = 10): array
    {
    ...
    private function searchWhere(array $where)
    {
        $model = $this->getModel();
        if (isset($where['pay_count']) && $where['pay_count'] != '') {
            $model = $model->where("pay_count", $where['pay_count']);
        } elseif (isset($where['status']) && $where['status'] != '') {
            $model = $model->where("status", $where['status']);
        } elseif (isset($where['last_ip']) && $where['last_ip'] != '') {
            $model = $model->where("last_ip", $where['last_ip']);
        }
        return $model;
    }
}
